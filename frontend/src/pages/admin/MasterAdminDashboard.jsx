...
export default function MasterAdminDashboard() {
  ...
  const handleDeleteQuiz = async (quizId) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this static quiz? This cannot be undone.",
      )
    ) {
      return;
    }
      ...
      <aside style={s.sidebar}>
        <div>
          ...
          <div style={s.sideNote}>
            Live rooms keep their realtime leaderboards. Static quizzes are now
            managed as practice libraries only.
          </div>
        </div>
        <div>
      ...
      <main style={s.main}>
        ...
        <div style={s.statGrid}>
          {[
            ...
            ["Static Packs", staticQuizzes.length],
        ...
        <section style={s.section}>
          <div style={s.sectionHead}>
            <div>
              <h2 style={s.sectionTitle}>Static Quiz Library</h2>
              <div style={s.sectionSub}>
                Placement and general practice packs with no persistent
                leaderboard state.
              </div>
            </div>
            <button style={s.primaryBtn} onClick={handleOpenCreateQuiz}>
              + Create Quiz
            </button>
          </div>
          ...
          <div style={s.quizSummaryGrid}>
            {staticQuizzes.map((quiz) => (
              <div key={quiz.quiz_id} style={s.quizSummaryCard}>
                <div style={s.quizSummaryTitle}>{quiz.title}</div>
                <div style={s.quizSummaryMeta}>
                  {quiz.genre} · {quiz.question_count || quiz.num_questions} linked
                  questions
                </div>
                <div style={s.quizSummaryTop}>
                  {quiz.num_questions} per attempt · {quiz.time_per_question}s per
                  question
                </div>
...
const s = {
  ...
  sectionSub: { marginTop: "0.35rem", color: "#91847a", fontSize: "0.8rem" },
  empty: {
    borderRadius: "12px",
    padding: "1rem",
    background: "#161111",
